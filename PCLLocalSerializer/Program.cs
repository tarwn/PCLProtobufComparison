...
namespace PCLLocalSerializer
{
    class Program
    {
        static void Main(string[] args)
        {
            var test = new LocalTestCore.Test() { Name = "PCL with Local Serializer" };

            var results = new List<LocalTestCore.Result>();
            for (int i = 0; i < 5; i++)
            ...
            File.WriteAllText("PCLLocalSerializer.csv", columns + data);
        }
    }
}
